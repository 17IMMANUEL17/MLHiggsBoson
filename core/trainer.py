...
from core.supplementary_implementations import (
    logistic_regression_bfgs,
    ...
    for model, to_train in cfg.model_selection.items():
        if to_train:
            ...
            for i in range(hyperparams.shape[0]):
                ...
                val_idx, train_ifx = kfold_cross_validation(
                    train_data["x_train"], num_folds
                )
                ...
                for val_fold, train_fold in zip(val_idx[:fold], train_ifx[:fold]):
                    ...
                    w, loss = train(
                        cfg, fold, hyperparams[i, :], model, w_initial, x_train, y_train
                    )
...
def train(cfg, fold, trial_hyperparams, model_name, w_initial, x_train, y_train):
    ...
    logging.info(
        f"Training the model: {model_name} for fold {fold} - started training at {start_time}"
    )

    # select and train the model based on the given model_name
    w, loss = choose_model(
        model_name, y_train, x_train, _lambda, w_initial, cfg.n_epochs, init_gamma
    )

    end_time = datetime.now().replace(microsecond=0)
    execution_time = (end_time - start_time).total_seconds()
    logging.info(
        f"Training of the model {model_name} for fold {fold} finished in {execution_time} seconds \n"
    )
    return w, loss
...
def evaluate(model_name, w, x_val, y_val):
    ...
    if len(np.array(w).shape) == 1:
        ...
        accuracy_eval = (
            np.mean(pred_eval == np.tile(y_val, (pred_eval.shape[1], 1)).T, axis=0)
            * 100
        )

    logging.info(
        f"Accuracy of the {model_name} on the validation set: {accuracy_eval} \n"
    )
...
def choose_best_result(
    cfg, hyperparams, hyperparam_acc, hyperparam_pred, model, loss, test_data
):
    ...
    logging.info(
        f"init_gamma: {init_gamma}; fin_gamma: {fin_gamma}; gamma_decay: {gamma_dec}; _lambda: {_lambda};"
    )
...
def choose_model(model_name, y_train, x_train, lambda_, w_initial, max_iters, gamma):
    ...
    if model_name == "mean_squared_error_gd":
        ...
        w, loss = reg_logistic_regression(
            y_train, x_train, lambda_, w_initial, max_iters, gamma
        )

    elif model_name == "logistic_regression_bfgs":
        # the best experimental degree for logistic_regression=3
        w, loss = logistic_regression_bfgs(
            y_train, x_train, w_initial, max_iters, gamma
        )

    elif model_name == "reg_logistic_regression_bfgs":
        # the best experimental degree for reg_logistic_regression=3
        w, loss = reg_logistic_regression_bfgs(
            y_train, x_train, lambda_, w_initial, max_iters, gamma
        )
